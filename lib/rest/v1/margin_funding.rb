...
module Bitfinex
  ...
  module RESTv1MarginFunding
    ...
    def new_offer(currency, amount, rate, period, direction, frrdelta = false) # rubocop:disable Metrics/ParameterLists
      params = {
        currency: currency,
        amount: amount.to_s,
        rate: rate.to_s,
    ...
    def cancel_offer(offer_id)
      authenticated_post('offer/cancel', params: {
                           offer_id: offer_id.to_i
                         }).body
    end
    ...
    def offer_status(offer_id)
      authenticated_post('offer/status', params: {
                           offer_id: offer_id.to_i
                         }).body
    end
    ...
    #
    # @return [Array] status for all your live offers (lending or borrowing)
    # @example:
    #   client.offers
    def offers
    ...
    # View your funding currently borrowed and not used
    # (available for a new margin position).
    #
    ...
    def close_funding(swap_id)
      authenticated_post('funding/close', params: {
                           swap_id: swap_id.to_i
                         }).body
    end
  end
end
